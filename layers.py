...
class MRNNFixDCell(nn.RNNCellBase):
    ...
    def __init__(self, input_size, hidden_size, output_size, K, bias=True):
        super(MRNNFixDCell, self).__init__(input_size, hidden_size, bias, num_chunks=1)
        self.K = K
...
class MRNNCell(nn.RNNCellBase):
    __constants__ = ['input_size', 'hidden_size', 'K', 'bias']
    def __init__(self, input_size, hidden_size, output_size, K, bias=True):
        super(MRNNCell, self).__init__(input_size, hidden_size, bias, num_chunks=1)  # weight_ih, weight_hh
        self.K = K
        self.input_size = input_size
        self.hidden_size = hidden_size
        self.output_size = output_size
