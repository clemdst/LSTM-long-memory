...
def multivar_pdg_m(seq, m):  # only compute periodogram for Fourier frequencies 2*pi*(1,...,m)/T
    k, T = seq.shape
    out = np.zeros((k, k, m), dtype=float)
...
def R_d(d, lam, pdg, verbose=False):
    k = pdg.shape[0]
    m = pdg.shape[2]
    ghat = G_hat(d, lam, pdg) + np.eye(k) * 1e-6
...
def compute_total_memory_fixed(seq):
    k, T = seq.shape
    # 1. Choix de m : m = sqrt(T) est le standard du papier [cite: 181]
    m = int(np.sqrt(T)) 
    ...
    d_hat = GSE['x']
    tot_mem = np.mean(d_hat)

    # 3. Calcul de la variance pour le test de mémoire totale [cite: 232, 811]
    # Pour p=1, la variance de d_bar est 1/(4m)
    # Pour p > 1, on utilise la forme quadratique de la matrice Omega
    if k == 1:
        asy_var = 1 / (4 * m)
    else:
        # Reconstruction de la matrice Omega pour le cas multidimensionnel 
        # Note: Nécessite l'estimation de G(d) via l'optimiseur
        G_inv = np.linalg.inv(GSE['G_hat']) # Supposant que multi_GSE retourne G_hat
        H = GSE['G_hat'] * G_inv # Produit de Hadamard [cite: 173]
        Omega = 2 * (np.eye(k) + H + (np.pi**2 / 4) * (H - np.eye(k)))
        # Variance de la somme des d_i : (1^T * Omega^-1 * 1) / m [cite: 232]
        asy_var = np.sum(np.linalg.inv(Omega)) / m

    # 4. Test d'hypothèse (H0: d=0 vs H1: d > 0) [cite: 251, 254]
    z = tot_mem / np.sqrt(asy_var)
    p_val = 1 - scst.norm.cdf(z)
